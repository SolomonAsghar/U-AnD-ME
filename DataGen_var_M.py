...
##########################################################################
########## MAKE DICTIONARIES ##########
univ_param_bounds = {'D_mean': (1e-12,15),
                     'D_var': 0.01,
                     'alpha_mean': (0,1.999),
                     'alpha_var': 0.01}

MSM_param_bounds = {'num_segs': (2,10),
                    'Pii': (0.9,0.999)} 
...
def GenerateTrueData_M(N=10000, return_padding_mask=False, max_traj_len=200):
    ...
    while True:
        ...
        FOV_model_labels = [OneHotEncoder(n) for n in range(5)]
        ...
        for fov_idx, (FOV, FOV_model_label) in enumerate(zip(FOVs, FOV_model_labels)):
            ...
            for traj in split_trajs:
                ...
                all_model_labels[traj_idx] = FOV_model_label    # drop in the model label
        ...
        if break_flag:
            break
    if return_padding_mask:
        return all_trajs, all_labels, all_model_labels, padding_mask
    else:
        return all_trajs, all_labels, all_model_labels
########## \MAKE DATASET ##########