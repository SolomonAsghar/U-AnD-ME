...
from DataGen_var import *
from Evaluation import *

def UnlimitedTrain(model, checkpoint_path, data_generator=GenerateTrueData, data_post_process=None, patience=3, data_gen_per_iteration=10000, prop_train=0.8, verbose=False, max_traj_len=200):
    '''
    ...
    while p > 0:
        ...
        Labels_trn_K = Labels_trn[:,:,1:2]
        Labels_trn_alpha = Labels_trn[:,:,2:3]
        Labels_trn_diff_class = Labels_trn[:,:,3:4]
        

        Trajs_val = Trajs[int(len(Trajs)*prop_train):]
        Labels_val = Labels[int(len(Labels)*prop_train):]
        Labels_val_CPs = Labels_val[:,:,:1]
        Labels_val_K = Labels_val[:,:,1:2]
        Labels_val_alpha = Labels_val[:,:,2:3]
        Labels_val_diff_class = Labels_val[:,:,3:4]
        ...
        model.fit(x=Trajs_trn,
                  y={"CPs": Labels_trn_CPs,
                     "K": Labels_trn_K,
                     "alpha": Labels_trn_alpha,
                     "diff_class": Labels_trn_diff_class},
                  validation_data=(Trajs_val, {"CPs": Labels_val_CPs,
                                               "K": Labels_val_K,
                                               "alpha": Labels_val_alpha,
                                               "diff_class": Labels_val_diff_class}),
                  epochs=500,
