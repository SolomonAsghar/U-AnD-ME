...
def UNet3P_var(filters, ConvBlock, BlockParams, SkipBlockParams, DecoderBlockParams, input_len=200): 
    '''
    ...
    ## FinalTransform
    x = tf.keras.layers.Conv1D(4, 1, activation='linear')(Decoder[b])

    CPs = x[:,:,:1]
    CPs = tf.keras.layers.Activation('sigmoid', name='CPs')(CPs)
    K = x[:,:,1:2]
    K = tf.keras.layers.Activation('linear', name='K')(K)
    alpha = x[:,:,2:3]
    alpha = tf.keras.layers.Activation('linear', name='alpha')(alpha)
    diff_class = x[:,:,3:4]
    diff_class = tf.keras.layers.Activation('linear', name='diff_class')(diff_class)
    
    model = tf.keras.models.Model(input_layer, [CPs, K, alpha, diff_class])
    return model