...
def UnlimitedTrain_M(model, checkpoint_path, data_generator=GenerateTrueData_M, data_post_process=None, patience=3, data_gen_per_iteration=10000, prop_train=0.8, verbose=False, max_traj_len=200):
    ...
    while p > 0:
        ...
        Trajs, Labels, Labels_M = data_generator(N=data_gen_per_iteration, max_traj_len=max_traj_len)
        ...
        Labels_trn_M = Labels_M[:int(len(Labels)*prop_train)]
        ...
        Labels_val_M = Labels_M[int(len(Labels)*prop_train):]
        
        # Use generated data until we overtraining
        early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_K_and_alpha_and_class_loss', patience=5, verbose=1,
